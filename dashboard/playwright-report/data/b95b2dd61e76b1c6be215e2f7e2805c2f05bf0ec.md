...
- Name: demo-flow.spec.ts >> successful mock GitHub connection and simulated project deployment
- Location: tests\e2e\demo-flow.spec.ts:4:5
...
Call log:
  - Expect "toHaveURL" with timeout 15000ms
    16 × unexpected value "http://localhost:3000/login"
        ...
        - generic [ref=e35]:
          - generic [ref=e36]:
            ...
            - textbox "Email" [ref=e37]:
              ...
              - text: e2e-1781459121149-l7jngh@example.com
          - generic [ref=e38]:
            - text: Password
...
```ts
  ...
  76 | 
  77 |   // Wait for either terms gate or dashboard to be loaded
> 78 |   await expect(page).toHaveURL(/\/(auth\/terms|dashboard)/, { timeout: 15_000 });
     |                      ^ Error: expect(page).toHaveURL(expected) failed
  79 | 
  80 |   if (page.url().includes("/auth/terms")) {
  81 |     const checkbox = page.getByRole("checkbox");
  82 |     await checkbox.waitFor({ state: "visible", timeout: 5000 });
  83 |     await checkbox.check();
  84 |     await page.getByRole("button", { name: /^continue$/i }).click();
  85 |   }
  86 | 
  87 |   await expect(page).toHaveURL(/\/dashboard/, { timeout: 15_000 });
  88 | }
  89 | 
```